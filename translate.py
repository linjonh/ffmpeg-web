...
async def handle_html_file(html: str, base_name: str):
    ...
    for node in childs:
        if isinstance(node, NavigableString) and not isinstance(node, Comment) and not isinstance(node, Doctype):
            parent = node.parent
            ...
            if chunk_size < 10_000:
                ...
                task.append(start_trans(text_array))
    ...
    for node,text in zip(childs,translate_arry):
...
if __name__ == "__main__":
    ...
    list_files = glob.glob(f"{docs_path}/about.html")
    ...
    for item in list_files:  # import os
        ...
        # break #先测试一个文件
            # result = call_chatgpt(prompt=f"翻译网页内容中的英文成中文，保留原始网页标签格式,且全部整个页面翻译，不要只翻译一半，网页是: {data}")
            # print(f"翻译后的样本：{result[:10]}")
            # if result is not None:
            #     fname=os.path.join(docs_path_cn,base_name)
            #     print(f"write {fname}")
