...
def append_task(task:list, text_array: dict[int, str]):
    ...
    json_str= json.dumps(text_array, ensure_ascii=False)
    prompt = f"翻译下数json的values文案，直接返回翻译后的json，不要输出markdown，要纯json文本格式输出：{json_str}"
    return task.append(call_chatgpt(prompt=prompt))
...
async def handle_html_file(html: str, base_name: str):
    ...
    for i, rep in enumerate(all_results):
        ...
        if rep is None:
            ...
            continue
        translate_arry.update(json.loads(rep))
    print(f"===> translate_arry={translate_arry}")
    for i,node in nodes.items:
        node:PageElement
        print(f"before {node}")
        text = translate_arry[i].strip()
